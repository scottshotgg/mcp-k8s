...
type ListAllPodsArgs struct {
	Namespace string `json:"namespace" jsonschema:"required,description=Namespace that the deployment should be deployed into in"`
}

func (k *KubernetesTool) ListAllPods(ctx context.Context, args ListAllPodsArgs) (*mcp_golang.ToolResponse, error) {
	fmt.Println("list_all_pods")
